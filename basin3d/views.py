...
def broker_api_root(request, format=None):
    ...
    # root api.
    views = []
    # If synthesis is turned on present the views
    if settings.BASIN3D["SYNTHESIS"]:
        views.extend(BASIN3D_SYNTHESIS_VIEWS)

    # if the direct api is turned on, present the views
    ...
    if settings.BASIN3D["SYNTHESIS"]:
        root_dict['synthesis-monitoringfeatures'] = \
            '{}://{}/synthesis/monitoringfeatures/'.format(request.scheme, request.get_host())
